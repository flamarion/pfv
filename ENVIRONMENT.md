...
| `REFRESH_IDLE_TTL_DAYS` | no | `30` | `.env` | unset | inherit default | no | Idle TTL for the refresh-cookie session. Single source of truth for the refresh JWT `exp` claim and the cookie `Max-Age` attribute (and, post PR 2 of the session-model rollout, the Redis `jti` key TTL). Validator enforces `1 <= v <= 365`; out-of-range values refuse to boot. | Default 30 days. |
| `SESSION_LIFETIME_DAYS` | no | `30` | `.env` | unset | inherit default | no | Absolute session ceiling. Independent from `REFRESH_IDLE_TTL_DAYS` (idle vs absolute). Per-org override via `OrgSetting(key="session_lifetime_days")`. | Default 30. |
