...
async def stop_recurring(db: AsyncSession, org_id: int, recurring_id: int) -> int:
    ...
    # Find and delete pending transactions generated by this template
    today = dt.date.today()
    ...
    pending_txs = list(pending_result.scalars().all())
...
async def delete_recurring(db: AsyncSession, org_id: int, recurring_id: int) -> int:
    ...
    # Delete any remaining pending future transactions
    today = dt.date.today()
    pending_result = await db.execute(
        select(Transaction).where(
            Transaction.recurring_id == recurring_id,
